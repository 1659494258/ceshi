...
const tableList = ref([])
const optionList = ref([]) //右侧下拉选择框的码值
const data = ref({
  ...props.dataDefault,
})
...
const getOutputList = () => {
  ...
  console.log(statement, outputConditionListAll.value)
...
const getInnerCondition = (obj: any) => {
  ...
  if (obj?._lop && obj?.logicalOperands.length > 0) {
    let statement = ''
    for (let i = 0; i <= obj?.logicalOperands.length - 1; i++) {
      statement +=
        (i === 0 ? '(' : '') +
        getInnerCondition(obj?.logicalOperands[i]) +
        (i === obj?.logicalOperands.length - 1 ? '' : obj?._lop) +
        (i === obj?.logicalOperands.length - 1 ? ')' : '')
    }
    return statement
  }
...
const firstChange = (e: any) => {
  ...
  // 判断第一个所选的类型
  if (first[0]?.info?._option_method) {
    codeById(first[0]?.id).then(res => {
      console.log('selected', res.data.message.codes[0].items)
      optionList.value = res.data.message.codes[0].items
    })
  } else {
    console.log('input')
  }
